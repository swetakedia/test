...
Crafting transactions and getting the base64-encoded transaction envelope (often referred to as the "transaction blob") is a central aspect of interacting with the test network.  The Go SDK uses the `build` package to craft transactions.  The example below builds a payment for testnet and outputs the encoded blob to standard out.  For this example, we have two previously created accounts on the test network.
...
func main() {
	...
	tx, err := b.Transaction(
		...
		b.TestNetwork,
		b.AutoSequence{SequenceProvider: testhorizon.DefaultTestNetClient},
		b.Payment(
...
func main() {
	...
	resp, err := testhorizon.DefaultTestNetClient.SubmitTransaction(blob)
	if err != nil {
		panic(err)
	}
