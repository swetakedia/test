...
* This TestHorizon version no longer supports Core <10.0.0. TestHorizon can still ingest version <10 ledgers.
...
* Added `testhorizon db reingest range X Y` that reingests ledgers between X and Y sequence number (closed intervals).
...
Previous versions work fine with TestHorizon 0.16.0 schema so you can migrate (`testhorizon db migrate up`) database without stopping the TestHorizon process. To reingest ledgers run `testhorizon db reingest` using TestHorizon 0.16.0 binary.
...
* Improved `testhorizon db migrate` script. It will now either success or show a detailed message regarding why it failed.
...
* Fixed parameter checking in `testhorizon db backfill` command.

## v0.15.3 - 2019-01-07

* Fixed a bug in TestHorizon DB reaping code.
* Fixed query checking code that generated `ERROR`-level log entries for invalid input.

## v0.15.2 - 2018-12-13

* Added `testhorizon db init-asset-stats` command to initialize `asset_stats` table. This command should be run once before starting ingestion if asset stats are enabled (`ENABLE_ASSET_STATS=true`).
...
* ["Trades"](https://www.test.org/developers/testhorizon/reference/endpoints/trades.html) endpoint can now be streamed.
* In ["Trade Aggregations"](https://www.test.org/developers/testhorizon/reference/endpoints/trade_aggregations.html) endpoint, `offset` parameter has been added.
* Path finding bugs have been fixed and the algorithm has been improved. Check [#719](https://github.com/test/go/pull/719) for more information.
* Connections (including streams) are closed after timeout defined using `--connection-timeout` CLI param or `CONNECTION_TIMEOUT` environment variable. If TestHorizon is behind a load balancer with idle timeout set, it is recommended to set this to a value equal a few seconds less than idle timeout so streams can be properly closed by TestHorizon.
...
* In ["Offers for Account"](https://www.test.org/developers/testhorizon/reference/endpoints/offers-for-account.html), `last_modified_time` field  endpoint can be `null` when ledger data is not available (has not been ingested yet).
* ["Trades for Offer"](https://www.test.org/developers/testhorizon/reference/endpoints/trades-for-offer.html) endpoint will query for trades that match the given offer on either side of trades, rather than just the "sell" offer. Offer IDs are now [synthetic](https://www.test.org/developers/testhorizon/reference/resources/trade.html#synthetic-offer-ids). You have to reingest history to update offer IDs.

### Other bug fixes

* `testhorizon db backfill` command has been fixed.
...
* Assets stats can cause high CPU usage on test-core DB. If this slows down the database it's now possible to turn off this feature by setting `DISABLE_ASSET_STATS` feature flag. This can be set as environment variable (`DISABLE_ASSET_STATS=true`) or CLI parameter (`--disable-asset-stats=true`).
...
* TestHorizon sends `Cache-Control: no-cache, no-store, max-age=0` HTTP header for all responses.
...
* Protocol V10 features:
  * New `bump_sequence` operation (as in [CAP-0001](https://github.com/test/test-protocol/blob/master/core/cap-0001.md)).
    * New [`bump_sequence`](https://www.test.org/developers/testhorizon/reference/resources/operation.html#bump-sequence) operation.
    * New `sequence_bumped` effect.
    * Please check [CAP-0001](https://github.com/test/test-protocol/blob/master/core/cap-0001.md) for new error codes for transaction submission.
  * Offer liabilities (as in [CAP-0003](https://github.com/test/test-protocol/blob/master/core/cap-0003.md)):
    * `/accounts/{id}` resources contain new fields: `buying_liabilities` and `selling_liabilities` for each entry in `balances`.
    * Please check [CAP-0003](https://github.com/test/test-protocol/blob/master/core/cap-0003.md) for new error codes for transaction submission.
...
- `amount` field in `/assets` is now a String (to support Test amounts larger than `int64`).

### Changes

- Effect resource contains a new `created_at` field.
- TestHorizon responses are compressed.
- Ingestion errors have been improved.
- `testhorizon rebase` command was improved.

### Bug fixes

- TestHorizon now returns `400 Bad Request` for negative `cursor` values.
...
- Fixed an issue caused by un-migrated trade rows. (https://github.com/test/go/issues/357)
- Command line flags are now useable for subcommands of testhorizon.


## v0.12.0 - 2017-03-08

Big release this time for testhorizon:  We've made a number of breaking changes since v0.11.0 and have revised both our database schema as well as our data ingestion system.  We recommend that you take a backup of your testhorizon database prior to upgrading, just in case.  
...
1. Upgrade testhorizon binaries, but do not restart the service
2. Run `testhorizon db migrate up` to migrate the db schema
3. Run `testhorizon db reingest` in a background session to begin the data reingestion process
4. Restart testhorizon

### Added

- Operation and payment resources were changed to add `transaction_hash` and `created_at` properties.
- The ledger resource was changed to add a `header_xdr` property.  Existing testhorizon installations should re-ingest all ledgers to populate the history database tables with the data.  In future versions of testhorizon we will disallow null values in this column.  Going forward, this change reduces the coupling of testhorizon to test-core, ensuring that testhorizon can re-import history even when the data is no longer stored within test-core's database.
...
- Changes to an account's inflation destination no longer produce erroneous "signer_updated" effects. (https://github.com/test/testhorizon/issues/390)
...
- Full reingestion (i.e. running `testhorizon db reingest`) now runs in reverse chronological order.  
...
- Upstream bug fixes to xdr decoding from `github.com/test/go`.
...
### Fixed
- Ingestion was fixed to protect against text memos that contain null bytes.  While memos with null bytes are legal in test-core, PostgreSQL does not support such values in string columns.  TestHorizon now strips those null bytes to fix the issue. 
...
### Added
- Added `testhorizon db clear` helper command to clear previously ingested history.
...
### Added
- TestHorizon now exposes the test network protocol in several places:  It shows the currently reported protocol version (as returned by the test-core `info` command) on the root endpoint, and it reports the protocol version of each ledger resource.
- Trade resources now include a `created_at` timestamp.

### Fixed

- BREAKING CHANGE: The reingestion process has been updated.  Prior versions of testhorizon would enter a failed state when a gap between the imported history and the test-core database formed or when a previously imported ledger was no longer found in the test-core database.  This usually occurs when running test-core with the `CATCHUP_RECENT` config option.  With these changed, testhorizon will automatically trim "abandonded" ledgers: ledgers that are older than the core elder ledger.
...
This release contains the initial implementation of the "Abridged History System".  It allows a testhorizon system to be operated without complete knowledge of the ledger's history.  With this release, testhorizon will start ingesting data from the earliest point known to the connected test-core instance, rather than ledger 1 as it behaved previously.  See the admin guide section titled "Ingesting test-core data" for more details.

### Added

- *Elder* ledgers have been introduced:  An elder ledger is the oldest ledger known to a db.  For example, the `core_elder_ledger` attribute on the root endpoint refers to the oldest known ledger stored in the connected test-core database.
...
- TestHorizon now reports the last ledger ingested to test-core using the `setcursor` command.
- Requests for data that precede the recorded window of history stored by testhorizon will receive a `410 Gone` http response to allow software to differentiate from other "not found" situations.
...
- BREAKING: `testhorizon_latest_ledger` has renamed to `history_latest_ledger`
- TestHorizon no longer needs to begin the ingestion of historical data from ledger sequence 1.  
...
- The `/accounts` endpoint, which lets a consumer page through the entire set of accounts in the ledger, has been removed.  The change from complete to an abridged history in testhorizon makes the endpoint mostly useless, and after consulting with the community we have decided to remove the endpoint.
...
- BREAKING: TestHorizon can now import data from test-core without the aid of the testhorizon-importer project.  This process is now known as "ingestion", and is enabled by either setting the `INGEST` environment variable to "true" or specifying "--ingest" on the launch arguments for the testhorizon process.  Only one process should be running in this mode for any given testhorizon database.
- Add `testhorizon db init`, used to install the latest bundled schema for the testhorizon database.
- Add `testhorizon db reingest` command, used to update outdated or corrupt testhorizon database information.  Admins may now use `testhorizon db reingest outdated` to migrate any old data when updated testhorizon.
...
- Add `testhorizon db migrate [up|down|redo]` commands, used for installing schema migrations.  This work is in service of porting the testhorizon-importer project directly to testhorizon.
...
[v0.11.0]: https://github.com/test/testhorizon/compare/v0.10.1...v0.11.0
[v0.10.1]: https://github.com/test/testhorizon/compare/v0.10.0...v0.10.1
[v0.10.0]: https://github.com/test/testhorizon/compare/v0.9.1...v0.10.0
[v0.9.1]: https://github.com/test/testhorizon/compare/v0.9.0...v0.9.1
[v0.9.0]: https://github.com/test/testhorizon/compare/v0.8.0...v0.9.0
[v0.8.0]: https://github.com/test/testhorizon/compare/v0.7.1...v0.8.0
[v0.7.1]: https://github.com/test/testhorizon/compare/v0.7.0...v0.7.1
[v0.7.0]: https://github.com/test/testhorizon/compare/v0.6.2...v0.7.0
[v0.6.2]: https://github.com/test/testhorizon/compare/v0.6.1...v0.6.2
[v0.6.1]: https://github.com/test/testhorizon/compare/v0.6.0...v0.6.1
[v0.6.0]: https://github.com/test/testhorizon/compare/v0.5.1...v0.6.0
[v0.5.1]: https://github.com/test/testhorizon/compare/v0.5.0...v0.5.1
[v0.5.0]: https://github.com/test/testhorizon/compare/v0.4.0...v0.5.0
[v0.4.0]: https://github.com/test/testhorizon/compare/v0.3.0...v0.4.0
[v0.3.0]: https://github.com/test/testhorizon/compare/v0.2.0...v0.3.0
[v0.2.0]: https://github.com/test/testhorizon/compare/v0.1.1...v0.2.0
[v0.1.1]: https://github.com/test/testhorizon/compare/v0.1.0...v0.1.1
