...
import (
	...
	testhorizon "github.com/test/go/services/testhorizon/internal"
	"github.com/test/go/services/testhorizon/internal/db2/schema"
	apkg "github.com/test/go/support/app"
	support "github.com/test/go/support/config"
	"github.com/test/go/support/log"
	"github.com/throttled/throttled"
)

var config testhorizon.Config

var rootCmd = &cobra.Command{
	Use:   "testhorizon",
	Short: "client-facing api server for the test network",
	Long:  "client-facing api server for the test network. It acts as the interface between Test Core and applications that want to access the Test network. It allows you to submit transactions to the network, check the status of accounts, subscribe to event streams and more.",
...
func checkMigrations() {
	...
	if len(migrationsToApplyUp) > 0 {
		...
		stdLog.Printf("A database migration is required to run this version (%v) of TestHorizon. Run \"testhorizon db migrate up\" to update your DB. Consult the Changelog (https://github.com/test/go/blob/master/services/testhorizon/CHANGELOG.md) for more information.", apkg.Version())
		os.Exit(1)
	}

	nMigrationsDown := schema.GetNumMigrationsDown(viper.GetString("db-url"))
	if nMigrationsDown > 0 {
		stdLog.Printf("A database migration DOWN to an earlier version of the schema is required to run this version (%v) of TestHorizon. Consult the Changelog (https://github.com/test/go/blob/master/services/testhorizon/CHANGELOG.md) for more information.", apkg.Version())
		stdLog.Printf("In order to migrate the database DOWN, using the HIGHEST version number of TestHorizon you have installed (not this binary), run \"testhorizon db migrate down %v\".", nMigrationsDown)
		os.Exit(1)
	}
}

// configOpts defines the complete flag configuration for testhorizon
// Add a new entry here to connect a new field in the testhorizon.Config struct
var configOpts = []*support.ConfigOption{
	&support.ConfigOption{
		...
		Required:  true,
		Usage:     "testhorizon postgres database to connect with",
	},
	&support.ConfigOption{
		Name:      "test-core-db-url",
		EnvVar:    "TEST_CORE_DATABASE_URL",
		ConfigKey: &config.TestCoreDatabaseURL,
		OptType:   types.String,
		Required:  true,
		Usage:     "test-core postgres database to connect with",
	},
	&support.ConfigOption{
		Name:      "test-core-url",
		ConfigKey: &config.TestCoreURL,
		OptType:   types.String,
		Required:  true,
		Usage:     "test-core to connect with (for http commands)",
	},
	&support.ConfigOption{
		Name:        "port",
	...
	&support.ConfigOption{
		...
		FlagDefault:    55,
		CustomSetValue: support.SetDuration,
		Usage:          "defines the timeout of connection after which 504 response will be sent or stream will be closed, if TestHorizon is behind a load balancer with idle connection timeout, this should be set to a few seconds less that idle timeout",
	},
	&support.ConfigOption{
	...
	&support.ConfigOption{
		...
		OptType:   types.String,
		Usage:     "redis key for storing rate limit data, useful when deploying a cluster of TestHorizons, ignored when redis-url is empty",
	},
	&support.ConfigOption{
		Name:      "redis-url",
	...
	&support.ConfigOption{
		...
		OptType:   types.String,
		Usage:     "TLS certificate file to use for securing connections to testhorizon",
	},
	&support.ConfigOption{
		Name:      "tls-key",
		ConfigKey: &config.TLSKey,
		OptType:   types.String,
		Usage:     "TLS private key file to use for securing connections to testhorizon",
	},
	&support.ConfigOption{
		Name:        "ingest",
		...
		OptType:     types.Bool,
		FlagDefault: false,
		Usage:       "causes this testhorizon process to ingest data from test-core into testhorizon's db",
	},
	&support.ConfigOption{
		...
		OptType:     types.Bool,
		FlagDefault: false,
		Usage:       "causes this testhorizon process to ingest failed transactions data",
	},
	&support.ConfigOption{
		...
		OptType:     types.Uint,
		FlagDefault: uint(0),
		Usage:       "the minimum number of ledgers to maintain within testhorizon's history tables.  0 signifies an unlimited number of ledgers will be retained",
	},
	&support.ConfigOption{
		...
		OptType:     types.Uint,
		FlagDefault: uint(0),
		Usage:       "the maximum number of ledgers the history db is allowed to be out of date from the connected test-core db before testhorizon considers history stale",
	},
	&support.ConfigOption{
		...
		OptType:     types.Bool,
		FlagDefault: false,
		Usage:       "causes the ingester to skip reporting the last imported ledger state to test-core",
	},
	&support.ConfigOption{
...
func initApp() *testhorizon.App {
	initConfig()
	return testhorizon.NewApp(config)
}
