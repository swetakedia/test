...
import (
	...
	hlog "github.com/test/go/support/log"
)

var dbCmd = &cobra.Command{
	Use:   "db [command]",
	Short: "commands to manage testhorizon's postgres db",
}

var dbBackfillCmd = &cobra.Command{
	Use:   "backfill [COUNT]",
	Short: "backfills testhorizon history for COUNT ledgers",
...
var dbInitAssetStatsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		cdb, err := db.Open("postgres", config.TestCoreDatabaseURL)
		if err != nil {
			log.Fatal(err)
		}
...
var dbInitCmd = &cobra.Command{
	...
	Long:  "init initializes the postgres database used by testhorizon.",
	Run: func(cmd *cobra.Command, args []string) {
...
var dbRebaseCmd = &cobra.Command{
	Use:   "rebase",
	Short: "rebases clears the testhorizon db and ingests the latest ledger segment from test-core",
	Long:  "...",
	Run: func(cmd *cobra.Command, args []string) {
		hlog.DefaultLogger.Logger.Level = config.LogLevel
		initConfig()
...
func ingestSystem(ingestConfig ingest.Config) *ingest.System {
	...
	return ingest.New(passphrase, config.TestCoreURL, cdb, hdb, ingestConfig)
}
