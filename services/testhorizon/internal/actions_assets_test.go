...
import (
	...
	"github.com/test/go/protocols/testhorizon/base"
...
func TestAssetsActions(t *testing.T) {
	testDomain := struct {
		...
		Toml: hal.NewLink("https://test.com/.well-known/test.toml"),
	}
	empty := struct {
		Toml hal.Link `json:"toml"`
	}{
	...
	BTCGateway := testhorizon.AssetStat{
		Links: testDomain,
		Asset: base.Asset{
		...
		Amount:      "100.9876000",
		NumAccounts: 1,
		Flags: testhorizon.AccountFlags{
			AuthRequired:  true,
			AuthRevocable: false,
		},
	}
	SCOTScott := testhorizon.AssetStat{
		Links: empty,
		Asset: base.Asset{
			Type:   "credit_alphanum4",
			Code:   "SCOT",
	...
	USDGateway := testhorizon.AssetStat{
		Links: testDomain,
		Asset: base.Asset{
			Type:   "credit_alphanum4",
			Code:   "USD",
	...
	testCases := []struct {
		...
		wantPrevious string
		wantNext     string
		wantItems    []testhorizon.AssetStat
	}{
		{
			"/assets",
			...
			[]testhorizon.AssetStat{BTCGateway, SCOTScott, USDGateway},
		},
		// limit
		{
			"/assets?limit=3",
			...
			[]testhorizon.AssetStat{BTCGateway},
		},
		// cursor
		{
			"/assets?cursor=0",
			...
			"/assets?order=asc&limit=10&cursor=USD_GC23QF2HUE52AMXUFUH3AYJAXXGXXV2VHXYYR6EYXETPKDXZSAW67XO4_credit_alphanum4",
			[]testhorizon.AssetStat{SCOTScott, USDGateway},
		}, {
			...
			"/assets?order=asc&limit=10&cursor=USD_GC23QF2HUE52AMXUFUH3AYJAXXGXXV2VHXYYR6EYXETPKDXZSAW67XO4_credit_alphanum4",
			[]testhorizon.AssetStat{USDGateway},
		}, {
		...
		// asset_code
		{
			...
			[]testhorizon.AssetStat{SCOTScott},
		},
		// asset_issuer
		{
			...
			[]testhorizon.AssetStat{BTCGateway, USDGateway},
		}, {
