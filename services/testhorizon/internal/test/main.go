...
// have any dependencies on testhorizon's packages.  think constants,
...
import (
	...
	"github.com/test/go/services/testhorizon/internal/logmetrics"
	tdb "github.com/test/go/services/testhorizon/internal/test/db"
...
// T provides a common set of functionality for each test in testhorizon
type T struct {
	T          *testing.T
...
// Database returns a connection to the testhorizon test database
//
// DEPRECATED:  use `TestHorizon()` from test/db package
func Database(t *testing.T) *sqlx.DB {
	return tdb.TestHorizon(t)
}

// DatabaseURL returns the database connection the url any test
// use when connecting to the history/testhorizon database
//
// DEPRECATED:  use `TestHorizonURL()` from test/db package
func DatabaseURL() string {
	return tdb.TestHorizonURL()
}
...
// LoadScenarioWithoutTestHorizon populates the test Test core database a with
// pre-created scenario.  Unlike `LoadScenario`, this
func LoadScenarioWithoutTestHorizon(scenarioName string) {
	loadScenario(scenarioName, false)
}

// OverrideLogger sets the default logger used by testhorizon to `l`.  This is used
...
// RestoreLogger restores the default testhorizon logger after it is overridden
...
func Start(t *testing.T) *T {
	...
	result.TestHorizonDB = Database(t)
	result.CoreDB = TestCoreDatabase(t)
...
// TestCoreDatabase returns a connection to the test core test database
//
// DEPRECATED:  use `TestCore()` from test/db package
func TestCoreDatabase(t *testing.T) *sqlx.DB {
	return tdb.TestCore(t)
}

// TestCoreDatabaseURL returns the database connection the url any test
// use when connecting to the test-core database
//
// DEPRECATED:  use `TestCoreURL()` from test/db package
func TestCoreDatabaseURL() string {
	return tdb.TestCoreURL()
}
