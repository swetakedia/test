...
import (
	...
	. "github.com/test/go/services/testhorizon/internal/db2/history"
	. "github.com/test/go/services/testhorizon/internal/test/trades"
...
func TestTradeActions_Index(t *testing.T) {
	...
	defer ht.Finish()
	var records []testhorizon.Trade
	var firstTrade testhorizon.Trade
	...
	if ht.Assert.Equal(200, w.Code) {
		...
		// 	ensure created_at is populated correctly
		l := history.Ledger{}
		hq := history.Q{Session: ht.TestHorizonSession()}
...
func testTradeAggregationPrices(t *HTTPT, record testhorizon.TradeAggregation) {
...
func TestTradeActions_Aggregation(t *testing.T) {
	...
	var records []testhorizon.TradeAggregation
	var record testhorizon.TradeAggregation
	var nextLink string
...
func TestTradeActions_IndexRegressions(t *testing.T) {
	t.Run("Regression:  https://github.com/test/go/services/testhorizon/internal/issues/318", func(t *testing.T) {
		ht := StartHTTPTest(t, "trades")
		defer ht.Finish()
	...
	t.Run("Regression for nil prices: https://github.com/test/go/issues/357", func(t *testing.T) {
		ht := StartHTTPTest(t, "trades")
		defer ht.Finish()
		...
		_ = ht.TestHorizonDB.MustExec("UPDATE history_trades SET price_n = NULL, price_d = NULL")
		w = ht.Get("/trades")
...
// https://github.com/test/go/issues/215
