...
import (
	...
	testhorizonContext "github.com/test/go/services/testhorizon/internal/context"
...
// App represents the root of the state of a testhorizon instance.
type App struct {
	config                       Config
	web                          *Web
	historyQ                     *history.Q
	...
	testhorizonConnGauge         metrics.Gauge
...
func NewApp(config Config) *App {
	a := &App{
		config:         config,
		testhorizonVersion: app.Version(),
...
// Serve starts the testhorizon web server, binding it to a socket, setting up
...
func (a *App) Serve() {
	...
	log.Infof("Starting testhorizon on %s (ingest: %v)", addr, a.config.Ingest)
...
// history portion of testhorizon's database.
func (a *App) HistoryQ() *history.Q {
	return a.historyQ
}

// TestHorizonSession returns a new session that loads data from the testhorizon
// database. The returned session is bound to `ctx`.
func (a *App) TestHorizonSession(ctx context.Context) *db.Session {
	return &db.Session{DB: a.historyQ.Session.DB, Ctx: ctx}
}

// CoreSession returns a new session that loads data from the test core
// database. The returned session is bound to `ctx`.
...
// UpdateTestCoreInfo updates the value of coreVersion,
// currentProtocolVersion, and coreSupportedProtocolVersion from the Test
// core API.
func (a *App) UpdateTestCoreInfo() {
	if a.config.TestCoreURL == "" {
		return
	}

	core := &testcore.Client{
		URL: a.config.TestCoreURL,
	}

	resp, err := core.Info(context.Background())
	if err != nil {
		log.Warnf("could not load test-core info: %s", err)
		return
	}

	// Check if NetworkPassphrase is different, if so exit TestHorizon as it can break the
	// state of the application.
	if resp.Info.Network != a.config.NetworkPassphrase {
		log.Errorf(
			"Network passphrase of test-core (%s) does not match TestHorizon configuration (%s). Exiting...",
			resp.Info.Network,
...
func (a *App) UpdateMetrics() {
	...
	a.testhorizonConnGauge.Update(int64(a.historyQ.Session.DB.Stats().OpenConnections))
...
// Tick triggers testhorizon to update all of it's background processes such as
// transaction submission, metrics, ingestion and reaping.
func (a *App) Tick() {
	var wg sync.WaitGroup
	log.Debug("ticking app")
	// update ledger state, operation fee state, and test-core info in parallel
	wg.Add(3)
	go func() { a.UpdateLedgerState(); wg.Done() }()
	go func() { a.UpdateOperationFeeStatsState(); wg.Done() }()
	go func() { a.UpdateTestCoreInfo(); wg.Done() }()
	wg.Wait()
...
func (a *App) Context(ctx context.Context) context.Context {
	return context.WithValue(ctx, &testhorizonContext.AppContextKey, a)
}
...
func AppFromContext(ctx context.Context) *App {
	...
	val, _ := ctx.Value(&testhorizonContext.AppContextKey).(*App)
	return val
}
