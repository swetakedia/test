...
// Config is the configuration for testhorizon.  It gets populated by the
// app's main function and is provided to NewApp.
type Config struct {
	DatabaseURL            string
	TestCoreDatabaseURL string
	TestCoreURL         string
	Port                   uint
	...
	LogglyToken       string
	LogglyTag         string
	// TLSCert is a path to a certificate file to use for testhorizon's TLS config
	TLSCert string
	// TLSKey is the path to a private key file to use for testhorizon's TLS config
	TLSKey string
	// Ingest toggles whether this testhorizon instance should run the data ingestion subsystem.
	Ingest bool
	...
	// history data to retain in the testhorizon database. For the purposes of
	...
	// out-of-date by before testhorizon begins to respond with an error to history
	// requests.
	StaleThreshold uint
	// SkipCursorUpdate causes the ingestor to skip reporting the "last imported
	// ledger" state to test-core.
