...
// reading rows from a Test Core db
package core
...
// Offer is row of data from the `offers` table from test-core
type Offer struct {
...
// Q is a helper struct on which to hang common queries against a test
// core database.
type Q struct {
	*db.Session
}
...
// Signer is a row of data from the `signers` table from test-core
type Signer struct {
	Accountid string
	Publickey string
	Weight    int32
...
// Transaction is row of data from the `txhistory` table from test-core
...
// TransactionFee is row of data from the `txfeehistory` table from test-core
...
// Trustline is a row of data from the `trustlines` table from test-core
...
// test-core database this ingestion system is communicating with.  TestHorizon,
