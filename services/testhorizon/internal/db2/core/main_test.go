...
func TestSchemaVersion8(t *testing.T) {
	tt := test.Start(t).ScenarioWithoutTestHorizon("core_database_schema_version_8")
	defer tt.Finish()
	q := &Q{tt.CoreSession()}
	...
	if tt.Assert.NoError(err) {
		...
		tt.Assert.Equal("test.org", account.HomeDomain.String)
	}
...
func TestSchemaVersion9(t *testing.T) {
	tt := test.Start(t).ScenarioWithoutTestHorizon("core_database_schema_version_9")
	defer tt.Finish()
	q := &Q{tt.CoreSession()}
