...
// Action is the "base type" for all actions in testhorizon.  It provides
...
// CoreQ provides access to queries that access the test core database.
...
// testhorizon's database.
func (action *Action) HistoryQ() *history.Q {
	if action.hq == nil {
		action.hq = &history.Q{Session: action.App.TestHorizonSession(action.R.Context())}
	}
