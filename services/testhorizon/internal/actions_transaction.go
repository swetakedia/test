...
import (
	...
	"github.com/test/go/services/testhorizon/internal/db2"
	...
	"github.com/test/go/services/testhorizon/internal/render/sse"
	...
	action.Do(
		...
		func() {
			...
			for _, record := range records {
				var res testhorizon.Transaction
...
func (action *TransactionIndexAction) loadParams() {
	...
	if action.IncludeFailed == true && !action.App.config.IngestFailedTransactions {
		err := errors.New("`include_failed` parameter is unavailable when TestHorizon is not ingesting failed " +
...
type TransactionShowAction struct {
	...
	Resource testhorizon.Transaction
}
...
// TransactionCreateAction submits a transaction to the test-core network
// on behalf of the requesting client.
type TransactionCreateAction struct {
	Action
	TX       string
	Result   txsub.Result
	Resource testhorizon.TransactionSuccess
}
...
func (action *TransactionCreateAction) loadResource() {
	...
	case *txsub.FailedTransactionError:
		rcr := testhorizon.TransactionResultCodes{}
		...
		action.Err = &problem.P{
			...
			Detail: "The transaction failed when submitted to the test network. " +
				"The `extras.result_codes` field on this response contains further " +
				"details.  Descriptions of each code can be found at: " +
				"https://www.test.org/developers/learn/concepts/list-of-operations.html",
	...
	case *txsub.MalformedTransactionError:
		action.Err = &problem.P{
			...
			Status: http.StatusBadRequest,
			Detail: "TestHorizon could not decode the transaction envelope in this " +
