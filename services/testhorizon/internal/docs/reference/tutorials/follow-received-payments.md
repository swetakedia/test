...
This tutorial shows how easy it is to use TestHorizon to watch for incoming payments on an [account](../../reference/resources/account.md)
using JavaScript and `EventSource`.  We will eschew using [`js-test-sdk`](https://github.com/test/js-test-sdk), the
...
```javascript
var Keypair = require("test-base").Keypair;
...
The Test test network provides the Friendbot, a tool that developers
can use to get testnet lumens for testing purposes. To fund your account, simply
execute the following curl command:

```bash
$ curl "https://friendbot.test.org/?addr=GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3"
```
...
After a few seconds, the Test network will perform consensus, close the
...
```bash
$ curl -H "Accept: text/event-stream" "https://testhorizon-testnet.test.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3/payments"
```
...
> If you need a reliable streaming connection please use our [SDK](https://github.com/test/js-test-sdk).
...
var es = new EventSource('https://testhorizon-testnet.test.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3/payments');
...
First, let's check our account sequence number so we can create a payment transaction. To do this we send a request to testhorizon:

```bash
$ curl "https://testhorizon-testnet.test.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3"
```
...
TestBase.Network.useTestNetwork();

var keypair = TestBase.Keypair.fromSecret('SCU36VV2OYTUMDSSU4EIVX4UUHY3XC7N44VL4IJ26IOG6HVNC7DY5UJO');
var account = new TestBase.Account(keypair.publicKey(), "713226564141056");

var amount = "100";
var transaction = new TestBase.TransactionBuilder(account)
  .addOperation(TestBase.Operation.createAccount({
    destination: TestBase.Keypair.random().publicKey(),
...
After running this script you should see a signed transaction blob. To submit this transaction we send it to testhorizon or test-core. But before we do, let's open a new console and start our previous script by `node stream_payments.js`.

Now to send a transaction just use testhorizon:

```bash
curl -H "Content-Type: application/json" -X POST -d '{"tx":""}' "https://testhorizon-testnet.test.org/transactions"
```
