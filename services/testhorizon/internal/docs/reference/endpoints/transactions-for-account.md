...
clientData:
  laboratoryUrl: https://www.test.org/laboratory/#explorer?resource=transactions&endpoint=for_account
---
...
This endpoint can also be used in [streaming](../streaming.md) mode so it is possible to use it to listen for new transactions that affect a given account as they get made in the Test network.
If called in streaming mode TestHorizon will start at the earliest known transaction unless a `cursor` is set. In that case it will start from the `cursor`. You can also set `cursor` value to `now` to only stream transaction created since your request time.
...
```sh
curl "https://testhorizon-testnet.test.org/accounts/GCEZWKCA5VLDNRLN3RPRJMRZOX3Z6G5CHCGSNFHEYVXM3XOJMDS674JZ/transactions?limit=1"
```
