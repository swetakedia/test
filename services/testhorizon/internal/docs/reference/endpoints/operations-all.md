...
clientData:
  laboratoryUrl: https://www.test.org/laboratory/#explorer?resource=operations&endpoint=all
---
...
This endpoint can also be used in [streaming](../streaming.md) mode so it is possible to use it to listen as operations are processed in the Test network.
If called in streaming mode TestHorizon will start at the earliest known operation unless a `cursor` is set. In that case it will start from the `cursor`. You can also set `cursor` value to `now` to only stream operations created since your request time.
...
```sh
curl "https://testhorizon-testnet.test.org/operations?limit=200&order=desc"
```
