...
clientData:
  laboratoryUrl: https://www.test.org/laboratory/#explorer?resource=effects&endpoint=for_account
---
...
This endpoint can also be used in [streaming](../streaming.md) mode so it is possible to use it to listen for new effects as transactions happen in the Test network.
If called in streaming mode TestHorizon will start at the earliest known effect unless a `cursor` is set. In that case it will start from the `cursor`. You can also set `cursor` value to `now` to only stream effects created since your request time.
...
```sh
curl "https://testhorizon-testnet.test.org/accounts/GA2HGBJIJKI6O4XEM7CZWY5PS6GKSXL6D34ERAJYQSPYA6X6AI7HYW36/effects"
```
