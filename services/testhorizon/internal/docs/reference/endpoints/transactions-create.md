...
clientData:
  laboratoryUrl: https://www.test.org/laboratory/#explorer?resource=transactions&endpoint=create
---

Posts a new [transaction](../resources/transaction.md) to the Test Network.
...
Test Network's ledger is a [complicated and asynchronous
process](https://www.test.org/developers/learn/concepts/transactions.html#life-cycle).
To reduce the complexity, testhorizon manages these asynchronous processes for the
client and will wait to hear results from the Test Network before returning
an HTTP response to a client.

Transaction submission to testhorizon aims to be
[idempotent](https://en.wikipedia.org/wiki/Idempotence#Computer_science_meaning):
a client can submit a given transaction to testhorizon more than once and testhorizon
will behave the same each time.  If the transaction has already been
successfully applied to the ledger, testhorizon will simply return the saved result
...
Information about [building transactions](https://www.test.org/developers/js-test-base/learn/building-transactions.html) in JavaScript.
