...
import (
	...
	"github.com/test/go/protocols/testhorizon/effects"
...
func TestEffectActions_Index(t *testing.T) {
	...
	t.Run("omnibus test", func(t *testing.T) {
		...
		// test streaming, regression for https://github.com/test/go/services/testhorizon/internal/issues/147
	...
	t.Run("Effect resource props", func(t *testing.T) {
		...
		if ht.Assert.Equal(200, w.Code) {
			...
			err := ht.TestHorizonDB.Get(&ledger2, "SELECT * FROM history_ledgers WHERE sequence = 2")
