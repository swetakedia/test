...
func initDbMetrics(app *App) {
	...
	app.testhorizonConnGauge = metrics.NewGauge()
	...
	app.metrics.Register("test_core.latest_ledger", app.coreLatestLedgerGauge)
	app.metrics.Register("history.open_connections", app.testhorizonConnGauge)
	app.metrics.Register("test_core.open_connections", app.coreConnGauge)
...
func init() {
	...
	appInit.Add("db-metrics", initDbMetrics, "metrics", "testhorizon-db", "core-db")
