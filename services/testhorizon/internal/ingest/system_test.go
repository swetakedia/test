...
func TestBackfill(t *testing.T) {
	...
	// ensure 1 ledger
	var found int
	err = tt.TestHorizonSession().GetRaw(&found, "SELECT COUNT(*) FROM history_ledgers")
	tt.Require.NoError(err)
...
func TestValidation(t *testing.T) {
	...
	sys := New(network.TestNetworkPassphrase, "", tt.CoreSession(), tt.TestHorizonSession(), Config{})
	...
	// missing prev
	_, err = tt.TestHorizonSession().ExecRaw(
	...
	// mismatched header in testhorizon
	_, err = tt.TestHorizonSession().ExecRaw(`
