...
import (
	...
	"github.com/test/go/meta"
	"github.com/test/go/services/testhorizon/internal/db2/history"
	"github.com/test/go/services/testhorizon/internal/ingest/participants"
	"github.com/test/go/support/errors"
	ilog "github.com/test/go/support/log"
	sTime "github.com/test/go/support/time"
...
func (is *Session) ingestEffects() {
	...
	case xdr.OperationTypeCreatePassiveOffer:
		...
		// KNOWN ISSUE:  test-core creates results for CreatePassiveOffer operations
...
func (is *Session) ingestTrades() {
	...
	for i, trade := range trades {
		// test-core will opportunisticly garbage collect invalid offers (in the
...
// reportCursorState makes an http request to the configured test-core server
// to report that it has finished processing the data being ingested.  This
// allows test-core to free that storage when next it runs its own
// maintenance.
func (is *Session) reportCursorState() error {
	...
	core := &testcore.Client{URL: is.TestCoreURL}
