// Package ingest contains the ingestion system for testhorizon.  This system takes
// data produced by the connected test-core database, transforms it and
// inserts it into the testhorizon database.
package ingest
...
import (
	...
	ilog "github.com/test/go/support/log"
...
const (
	...
	// algorithm. As rows are ingested into the testhorizon database, this version is
...
// Cursor iterates through a test core database's ledgers
type Cursor struct {
	...
	// CoreDB is the test-core db that data is ingested from.
	CoreDB *db.Session
...
// System represents the data ingestion subsystem of testhorizon.
type System struct {
	// Config allows passing some configuration values to System.
	Config Config
	// TestHorizonDB is the connection to the testhorizon database that ingested data will
	// be written to.
	...
	Network string
	// TestCoreURL is the http endpoint of the test-core that data is being
	// ingested from.
	TestCoreURL string
...
type Ingestion struct {
	// DB is the sql connection to be used for writing any rows into the testhorizon
	// database.
	DB       *db.Session
...
type Session struct {
	...
	// ingested from.
	TestCoreURL string
	// ClearExisting causes the session to clear existing data from the testhorizon db
...
// New initializes the ingester, causing it to begin polling the test-core
// database for now ledgers and ingesting data into the testhorizon database.
func New(network string, coreURL string, core, testhorizon *db.Session, config Config) *System {
	i := &System{
		Config:         config,
		Network:        network,
		TestCoreURL: coreURL,
