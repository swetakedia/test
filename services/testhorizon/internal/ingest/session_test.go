...
import (
	...
	protocolEffects "github.com/test/go/protocols/testhorizon/effects"
...
func Test_ingestSignerEffects(t *testing.T) {
	tt := test.Start(t).ScenarioWithoutTestHorizon("set_options")
	defer tt.Finish()

	s := ingest(tt, Config{EnableAssetStats: false})
	tt.Require.NoError(s.Err)

	q := &history.Q{Session: tt.TestHorizonSession()}

	// Regression: https://github.com/test/testhorizon/issues/390 doesn't produce a signer effect when
...
func Test_ingestOperationEffects(t *testing.T) {
	...
	tt.ScenarioWithoutTestHorizon("kahuna")
	s = ingest(tt, Config{EnableAssetStats: false})
	tt.Require.NoError(s.Err)
