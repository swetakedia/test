...
import (
	...
	testDB "github.com/test/go/services/testhorizon/internal/test/db"
...
func TestAssetStatsIngest(t *testing.T) {
	tt := test.Start(t).ScenarioWithoutTestHorizon("ingest_asset_stats")
	defer tt.Finish()
	s := ingest(tt, Config{EnableAssetStats: true})
	tt.Require.NoError(s.Err)
	q := history.Q{Session: s.Ingestion.DB}
	...
	tt.Assert.Equal(AssetStatResult{
		...
		NumAccounts: 1,
		Flags:       1,
		Toml:        "https://test.com/.well-known/test.toml",
	}, assetStats[0])
...
func TestIngestFailedTransactionsEnabled(t *testing.T) {
	tt := test.Start(t).ScenarioWithoutTestHorizon("failed_transactions")
	defer tt.Finish()
...
func TestTradeIngestTimestamp(t *testing.T) {
	...
	tt := test.Start(t).ScenarioWithoutTestHorizon("trades")
	defer tt.Finish()
	s := ingest(tt, Config{EnableAssetStats: false})
	q := history.Q{Session: s.Ingestion.DB}
