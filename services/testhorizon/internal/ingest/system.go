...
import (
	...
	herr "github.com/test/go/services/testhorizon/internal/errors"
	"github.com/test/go/services/testhorizon/internal/ledger"
	"github.com/test/go/services/testhorizon/internal/toid"
...
// testhorizon elder query for `n` ledgers
...
// ClearAll removes all previously ingested historical data from the testhorizon
// database.
...
// RebaseHistory re-establishes testhorizon's history database by clearing it, ingesting the latest ledger in test-core then backfilling as many ledgers as possible
...
func (i *System) ReingestAll() (int, error) {
	...
	var elder int32
	var latest int32
	q := history.Q{Session: i.TestHorizonDB}
...
// run causes the importer to check test-core to see if we can import new
// data.
func (i *System) runOnce() {
	...
	historyQ := history.Q{Session: i.TestHorizonDB}
	...
	if coreLatest == 1 {
		log.Warn("ingest: waiting for test-core sync")
		return
	}
...
// testhorizon but is no longer present in the test-core database source.  The
// usual cause for this situation is a test-core that uses the CATCHUP_RECENT
// mode.
...
// within testhorizon.  It ensures the ledger at `seq` is a child of `seq - 1`.
func (i *System) validateLedgerChain(seq int32) error {
	...
	cq := &core.Q{Session: i.CoreDB}
	hq := &history.Q{Session: i.TestHorizonDB}
