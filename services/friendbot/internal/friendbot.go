...
type TxResult struct {
	maybeTransactionSuccess *testhorizon.TransactionSuccess
	maybeErr                error
}
...
func (bot *Bot) Pay(destAddress string) (*testhorizon.TransactionSuccess, error) {
...
func AsyncSubmitTransaction(bot *Bot, channel chan TxResult, signed string) {
	result, err := bot.TestHorizon.SubmitTransaction(signed)
	if err != nil {
		switch e := err.(type) {
		case *testhorizon.Error:
...
func (bot *Bot) checkHandleBadSequence(err *testhorizon.Error) {
...
func (bot *Bot) refreshSequence() error {
	botAccount, err := bot.TestHorizon.LoadAccount(bot.address())
	if err != nil {
		bot.sequence = 0
		return err
	}
