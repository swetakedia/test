...
func (pl *PaymentListener) Listen() (err error) {
	...
	_, err = pl.testhorizon.LoadAccount(accountID)
	if err != nil {
		return
	}
	...
	go func() {
		for {
			...
			var cursor testhorizon.Cursor
			if cursorValue != nil {
				cursor = testhorizon.Cursor(*cursorValue)
			} else {
				// If no last cursor saved set it to: `now`
				cursor = testhorizon.Cursor("now")
			}
			...
			err = pl.testhorizon.StreamPayments(context.Background(), accountID, &cursor, pl.onPayment)
			if err != nil {
...
func (pl *PaymentListener) ReprocessPayment(payment testhorizon.Payment, force bool) error {
...
func (pl *PaymentListener) onPayment(payment testhorizon.Payment) {
...
func (pl *PaymentListener) shouldProcessPayment(payment testhorizon.Payment) (bool, string) {
...
func (pl *PaymentListener) process(payment testhorizon.Payment) error {
	if payment.Type == "account_merge" {
		payment.AssetType = "native"
		payment.From = payment.Account
		payment.To = payment.Into

		err := pl.testhorizon.LoadAccountMergeAmount(&payment)
		if err != nil {
			return errors.Wrap(err, "Unable to load account_merge amount")
		}
	}

	err := pl.testhorizon.LoadMemo(&payment)
	if err != nil {
