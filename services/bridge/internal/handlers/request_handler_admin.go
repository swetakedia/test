...
func (rh *RequestHandler) AdminReceivedPayment(w http.ResponseWriter, r *http.Request) {
	...
	paymentResponse, err := rh.TestHorizon.LoadOperation(payment.OperationID)
	if err != nil {
		log.WithFields(log.Fields{"err": err}).Error("Error getting operation from TestHorizon")
		helpers.Write(w, helpers.InternalServerError)
		return
	}

	err = rh.TestHorizon.LoadMemo(&paymentResponse)
	if err != nil {
	...
	response := struct {
		...
		Operation testhorizon.Payment      `json:"operation"`
