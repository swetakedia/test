...
import (
	...
	"github.com/test/go/services/internal/bridge-compliance-shared/protocols/bridge"
...
func (rh *RequestHandler) standardPayment(w http.ResponseWriter, request *bridge.PaymentRequest) {
	...
	if request.ID != "" {
		...
		if sentTransaction == nil {
			...
			submitResponse, err := rh.TestHorizon.SubmitTransaction(sentTransaction.EnvelopeXdr)
			if err != nil {
	...
	if request.AssetCode != "" && request.AssetIssuer != "" {
		...
		_, err = rh.TestHorizon.LoadAccount(destinationObject.AccountID)
		if err != nil {
...
func (rh *RequestHandler) handleTransactionSubmitResponse(w http.ResponseWriter, submitResponse testhorizon.TransactionSuccess, err error) {
