...
import (
	...
	"github.com/test/go/build"
	"github.com/test/go/clients/testhorizon"
	"github.com/test/go/keypair"
	"github.com/test/go/services/bridge/internal/db"
	shared "github.com/test/go/services/internal/bridge-compliance-shared"
...
type TransactionSubmitterInterface interface {
	SubmitTransaction(paymentID *string, seed string, operation, memo interface{}) (response testhorizon.TransactionSuccess, err error)
	SignAndSubmitRawTransaction(paymentID *string, seed string, tx *xdr.Transaction) (response testhorizon.TransactionSuccess, err error)
}

// TransactionSubmitter submits transactions to Test Network
type TransactionSubmitter struct {
	TestHorizon       testhorizon.ClientInterface
...
// LoadAccount loads current state of Test account and creates a map entry if it didn't exist
func (ts *TransactionSubmitter) LoadAccount(seed string) (*Account, error) {
	...
	accountResponse, err := ts.TestHorizon.LoadAccount(ts.Accounts[seed].Keypair.Address())
	if err != nil {
		return nil, err
	}
...
func (ts *TransactionSubmitter) SignAndSubmitRawTransaction(paymentID *string, seed string, tx *xdr.Transaction) (response testhorizon.TransactionSuccess, err error) {
	...
	var herr *testhorizon.Error
	response, err = ts.TestHorizon.SubmitTransaction(txeB64)
	if err == nil {
		...
		var isTestHorizonError bool
		herr, isTestHorizonError = err.(*testhorizon.Error)
	...
	if herr != nil {
		...
		var accountResponse testhorizon.Account
		accountResponse, err = ts.TestHorizon.LoadAccount(account.Keypair.Address())
		if err != nil {
...
// SubmitTransaction builds and submits transaction to Test network
func (ts *TransactionSubmitter) SubmitTransaction(paymentID *string, seed string, operation, memo interface{}) (testhorizon.TransactionSuccess, error) {
	account, err := ts.LoadAccount(seed)
	if err != nil {
		return testhorizon.TransactionSuccess{}, errors.Wrap(err, "Error loading an account")
	}
	...
	if !ok {
		...
		return testhorizon.TransactionSuccess{}, errors.New("Cannot cast operationMutator to build.TransactionMutator")
	}
	...
	if memo != nil {
		...
		if !ok {
			...
			return testhorizon.TransactionSuccess{}, errors.New("Cannot cast memo to build.TransactionMutator")
		}
	...
	if err != nil {
		return testhorizon.TransactionSuccess{}, errors.Wrap(err, "Error building a transaction")
	}
