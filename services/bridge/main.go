...
import (
	...
	"github.com/test/go/services/bridge/internal/handlers"
	...
	"github.com/test/go/support/db/schema"
	"github.com/test/go/support/errors"
	supportHttp "github.com/test/go/support/http"
)
...
func init() {
	rootCmd = &cobra.Command{
		Use:   "bridge",
		Short: "test bridge server",
		Long:  `test bridge server`,
		Run:   run,
	}
...
func NewApp(config config.Config, migrateFlag bool, versionFlag bool, version string) (app *App, err error) {
	...
	h := testhorizon.Client{
		URL:  config.TestHorizon,
	...
	testtomlClient := testtoml.Client{
		HTTP: &httpClientWithTimeout,
	}

	federationClient := federation.Client{
		HTTP:        &httpClientWithTimeout,
		TestTOML: &testtomlClient,
	}

	err = g.Provide(
		&inject.Object{Value: &requestHandler},
		&inject.Object{Value: &config},
		&inject.Object{Value: &testtomlClient},
