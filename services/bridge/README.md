# bridge-server
This is a stand alone server written in go. It is designed to make connecting to the Test network as easy as possible. 
...
- Monitoring a receiving Test account.


## Downloading the server
[Prebuilt binaries](https://github.com/test/bridge-server/releases) of the bridge-server server are available on the [releases page](https://github.com/test/bridge-server/releases).

| Platform       | Binary file name                                                                         |
|----------------|------------------------------------------------------------------------------------------|
| Mac OSX 64 bit | [bridge-darwin-amd64](https://github.com/test/bridge-server/releases)      |
| Linux 64 bit   | [bridge-linux-amd64](https://github.com/test/bridge-server/releases)       |
| Windows 64 bit | [bridge-windows-amd64.exe](https://github.com/test/bridge-server/releases) |

Alternatively, you can [build](#building) the binary yourself.

## Config

The `bridge.cfg` file must be present in a working directory (you can load another file by using `-c` parameter). Here is an [example configuration file](https://github.com/test/bridge-server/blob/master/bridge_example.cfg). Config file should contain following values:
...
* `network_passphrase` - passphrase of the network that will be used with this bridge server:
   ...
   * public network: `Public Global Test Network ; September 2015`
* `compliance` - URL to compliance server instance if you want to carry out the compliance protocol
* `testhorizon` - URL to [testhorizon](https://github.com/test/testhorizon) server instance
...
* `mac_key` - a test secret key used to add MAC headers to a payment notification.
...
* [`InternalServerError`](/src/github.com/test/gateway/protocols/errors.go)

### POST /builder

Builds a transaction from a given request. `Content-Type` of this request should be `application/json`. Check [List of operations](https://www.test.org/developers/learn/concepts/list-of-operations.html) doc to learn more about how each operation looks like.

**Note** This will not submit a transaction to the network. Please use [TestHorizon](https://www.test.org/developers/testhorizon/reference/endpoints/transactions-create.html) to submit a transaction.
...
Assets are represented by a JSON object with two fields: `code` and `issuer`. Empty JSON object represents [native asset](https://www.test.org/developers/learn/concepts/assets.html#lumens-xlm-).
...
`sender` | optional | Payment address (ex. `bob*test.org`) of payment sender account. Required for when sending using Compliance protocol.
`destination` | required | Account ID or payment address (ex. `bob*test.org`) of payment destination account
...
`extra_memo` | optional | You can include any info here and it will be included in the pre-image of the transaction's memo hash. See the [Test Memo Convention](https://github.com/test/test-protocol/issues/28). When set and compliance server is connected, `memo` and `memo_type` values will be ignored.
...
```
forward_destination[domain]=test.org&forward_destination[fields][forward_type]=bank_account&forward_destination[fields][swift]=BOPBPHMM&forward_destination[fields][acct]=2382376
```

will be translate to the following request:

```
https://FEDERATION_SERVER_READ_FROM_TEST_TOML/federation?type=forward&forward_type=bank_account&swift=BOPBPHMM&acct=2382376
```

#### Response

It will return [`SubmitTransactionResponse`](/src/github.com/test/gateway/testhorizon/submit_transaction_response.go) if there were no errors or with one of the following errors:

* [`InternalServerError`](/src/github.com/test/gateway/protocols/errors.go)
* [`InvalidParameterError`](/src/github.com/test/gateway/protocols/errors.go)
* [`MissingParameterError`](/src/github.com/test/gateway/protocols/errors.go)
* [`TransactionBadSequence`](/src/github.com/test/gateway/protocols/bridge/errors.go)
* [`TransactionBadAuth`](/src/github.com/test/gateway/protocols/bridge/errors.go)
* [`TransactionInsufficientBalance`](/src/github.com/test/gateway/protocols/bridge/errors.go)
* [`TransactionNoAccount`](/src/github.com/test/gateway/protocols/bridge/errors.go)
* [`TransactionInsufficientFee`](/src/github.com/test/gateway/protocols/bridge/errors.go)
* [`TransactionBadAuthExtra`](/src/github.com/test/gateway/protocols/bridge/errors.go)
* [`PaymentCannotResolveDestination`](/src/github.com/test/gateway/protocols/bridge/payment.go)
* [`PaymentCannotUseMemo`](/src/github.com/test/gateway/protocols/bridge/payment.go)
* [`PaymentSourceNotExist`](/src/github.com/test/gateway/protocols/bridge/payment.go)
* [`PaymentAssetCodeNotAllowed`](/src/github.com/test/gateway/protocols/bridge/payment.go)
* [`PaymentPending`](/src/github.com/test/gateway/protocols/bridge/payment.go)
* [`PaymentDenied`](/src/github.com/test/gateway/protocols/bridge/payment.go)
* [`PaymentMalformed`](/src/github.com/test/gateway/protocols/bridge/payment.go)
* [`PaymentUnderfunded`](/src/github.com/test/gateway/protocols/bridge/payment.go)
* [`PaymentSrcNoTrust`](/src/github.com/test/gateway/protocols/bridge/payment.go)
* [`PaymentSrcNotAuthorized`](/src/github.com/test/gateway/protocols/bridge/payment.go)
* [`PaymentNoDestination`](/src/github.com/test/gateway/protocols/bridge/payment.go)
* [`PaymentNoTrust`](/src/github.com/test/gateway/protocols/bridge/payment.go)
* [`PaymentNotAuthorized`](/src/github.com/test/gateway/protocols/bridge/payment.go)
* [`PaymentLineFull`](/src/github.com/test/gateway/protocols/bridge/payment.go)
* [`PaymentNoIssuer`](/src/github.com/test/gateway/protocols/bridge/payment.go)
* [`PaymentTooFewOffers`](/src/github.com/test/gateway/protocols/bridge/payment.go)
* [`PaymentOfferCrossSelf`](/src/github.com/test/gateway/protocols/bridge/payment.go)
* [`PaymentOverSendmax`](/src/github.com/test/gateway/protocols/bridge/payment.go)
...
It will build and submits a transaction with a [`allow_trust`](https://www.test.org/developers/learn/concepts/list-of-operations.html#allow-trust) operation. 
The source of this transaction will be the account specified by `accounts.authorizing_seed` config parameter. 
You should make sure that this account is a low weight signer on the issuing account. See [Multi-sig](https://www.test.org/developers/learn/concepts/multi-sig.html) for more information. 
...
* [`TransactionInsufficientFee`](/src/github.com/test/gateway/protocols/bridge/errors.go)
* [`TransactionBadAuthExtra`](/src/github.com/test/gateway/protocols/bridge/errors.go)
* [`AllowTrustMalformed`](/src/github.com/test/gateway/protocols/bridge/authorize.go)
* [`AllowTrustNoTrustline`](/src/github.com/test/gateway/protocols/bridge/authorize.go)
* [`AllowTrustTrustNotRequired`](/src/github.com/test/gateway/protocols/bridge/authorize.go)
* [`AllowTrustCantRevoke`](/src/github.com/test/gateway/protocols/bridge/authorize.go)
...
name |  | description
--- | --- | ---
`operation_id` | required | TestHorizon ID of operation to reprocess
...
`data` | Value of the [AuthData](https://www.test.org/developers/learn/integration-guides/compliance-protocol.html). This field will be empty when compliance server is not connected.
...
Then simply open:
```
http://localhost:6060/pkg/github.com/test/gateway/
```
in a browser.
