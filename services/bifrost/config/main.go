...
type Config struct {
	...
	Test struct {
		TestHorizon           string `valid:"required" toml:"testhorizon"`
		...
		IssuerPublicKey string `valid:"required,test_accountid" toml:"issuer_public_key"`
		...
		DistributionPublicKey string `valid:"required,test_accountid" toml:"distribution_public_key"`
		// SignerSecretKey is:
		...
		// https://www.test.org/developers/guides/channels.html
		// Signer's sequence number will be consumed in transaction's sequence number.
		SignerSecretKey string `valid:"required,test_seed" toml:"signer_secret_key"`
		...
		StartingBalance string `valid:"optional,test_amount" toml:"starting_balance"`
...
func (c Config) SignerPublicKey() string {
	kp := keypair.MustParse(c.Test.SignerSecretKey)
	return kp.Address()
}
