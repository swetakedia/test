...
func (s *Server) Start() error {
	...
	s.TestAccountConfigurator.OnAccountCreated = s.onTestAccountCreated
	s.TestAccountConfigurator.OnExchanged = s.onExchanged
	s.TestAccountConfigurator.OnExchangedTimelocked = s.OnExchangedTimelocked
	...
	err := s.TestAccountConfigurator.Start()
	if err != nil {
		return errors.Wrap(err, "Error starting TestAccountConfigurator")
	}
...
func (s *Server) handlerGenerateAddress(w stdhttp.ResponseWriter, r *stdhttp.Request, chain database.Chain) {
	...
	testPublicKey := r.PostFormValue("test_public_key")
	_, err := keypair.Parse(testPublicKey)
	if err != nil || (err == nil && testPublicKey[0] != 'G') {
		log.WithField("testPublicKey", testPublicKey).Warn("Invalid testPublicKey")
		w.WriteHeader(stdhttp.StatusBadRequest)
		return
	}
	...
	err = s.Database.CreateAddressAssociation(chain, testPublicKey, address, index)
	if err != nil {
