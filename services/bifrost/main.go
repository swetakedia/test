...
import (
	...
	"github.com/test/go/services/bifrost/server"
	"github.com/test/go/services/bifrost/sse"
	"github.com/test/go/services/bifrost/test"
	"github.com/test/go/services/bifrost/stress"
	supportConfig "github.com/test/go/support/config"
...
var rootCmd = &cobra.Command{
	Use:   "bifrost",
	Short: "Bridge server to allow participating in Test based ICOs using Bitcoin and Ethereum",
}
...
This command will create 3 server.Server's listening on ports 8000-8002.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		for i := 0; i < numServers; i++ {
			go func(i int) {
				cfg.Port = ports[i]
				cfg.Test.SignerSecretKey = signers[i]
		...
		users := stress.Users{
			TestHorizon: &testhorizon.Client{
				URL: cfg.Test.TestHorizon,
				HTTP: &http.Client{
					Timeout: 60 * time.Second,
				},
			},
			NetworkPassphrase: cfg.Test.NetworkPassphrase,
			UsersPerSecond:    usersPerSecond,
			BifrostPorts:      ports,
			IssuerPublicKey:   cfg.Test.IssuerPublicKey,
		}
...
func init() {
	// TODO I think these should be default in test/go:
...
func createServer(cfg config.Config, stressTest bool) *server.Server {
	...
	testAccountConfigurator := &test.AccountConfigurator{
		NetworkPassphrase:     cfg.Test.NetworkPassphrase,
		IssuerPublicKey:       cfg.Test.IssuerPublicKey,
		DistributionPublicKey: cfg.Test.DistributionPublicKey,
		SignerSecretKey:       cfg.Test.SignerSecretKey,
		NeedsAuthorize:        cfg.Test.NeedsAuthorize,
		TokenAssetCode:        cfg.Test.TokenAssetCode,
		StartingBalance:       cfg.Test.StartingBalance,
		LockUnixTimestamp:     cfg.Test.LockUnixTimestamp,
	}

	if cfg.Test.StartingBalance == "" {
		testAccountConfigurator.StartingBalance = "2.1"
	}

	if cfg.Bitcoin != nil {
		testAccountConfigurator.TokenPriceBTC = cfg.Bitcoin.TokenPrice
	}

	if cfg.Ethereum != nil {
		testAccountConfigurator.TokenPriceETH = cfg.Ethereum.TokenPrice
	}

	testhorizonClient := &testhorizon.Client{
		URL: cfg.Test.TestHorizon,
		HTTP: &http.Client{
	...
	err = g.Provide(
		...
		&inject.Object{Value: testhorizonClient},
		&inject.Object{Value: server},
		&inject.Object{Value: sseServer},
		&inject.Object{Value: testAccountConfigurator},
	)
	if err != nil {
