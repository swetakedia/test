...
func (u *Users) Start(accounts chan<- server.GenerateAddressResponse) {
	...
	go func() {
		...
		err := u.TestHorizon.StreamPayments(context.Background(), u.IssuerPublicKey, &cursor, u.onNewPayment)
		if err != nil {
			panic(err)
		}
	}()
...
func (u *Users) onNewPayment(payment testhorizon.Payment) {
...
func (u *Users) newUser(kp *keypair.Full) server.GenerateAddressResponse {
	...
	params := url.Values{}
	params.Add("test_public_key", kp.Address())
	...
	go func() {
		...
		_, err = u.TestHorizon.SubmitTransaction(txeB64)
		if err != nil {
			fmt.Println(txeB64)
			panic(err)
		}
		...
		_, err = u.TestHorizon.SubmitTransaction(txeB64)
		if err != nil {
			if herr, ok := err.(*testhorizon.Error); ok {
...
func (u *Users) getAccount(account string) (testhorizon.Account, bool, error) {
	var hAccount testhorizon.Account
	hAccount, err := u.TestHorizon.LoadAccount(account)
	if err != nil {
