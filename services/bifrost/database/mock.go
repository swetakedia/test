...
func (m *MockDatabase) CreateAddressAssociation(chain Chain, testAddress, address string, addressIndex uint32) error {
	a := m.Called(chain, testAddress, address, addressIndex)
	return a.Error(0)
}
...
func (m *MockDatabase) GetAssociationByTestPublicKey(testPublicKey string) (*AddressAssociation, error) {
	a := m.Called(testPublicKey)
	return a.Get(0).(*AddressAssociation), a.Error(1)
}
