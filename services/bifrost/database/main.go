...
type Database interface {
	// CreateAddressAssociation creates Bitcoin/Ethereum-Test association. `addressIndex`
	// is the chain (Bitcoin/Ethereum) address derivation index (BIP-32).
	CreateAddressAssociation(chain Chain, testAddress, address string, addressIndex uint32) error
	// GetAssociationByChainAddress searches for previously saved Bitcoin/Ethereum-Test association.
	// Should return nil if not found.
	GetAssociationByChainAddress(chain Chain, address string) (*AddressAssociation, error)
	// GetAssociationByTestPublicKey searches for previously saved Bitcoin/Ethereum-Test association.
	// Should return nil if not found.
	GetAssociationByTestPublicKey(testPublicKey string) (*AddressAssociation, error)
...
type AddressAssociation struct {
	...
	TestPublicKey string    `db:"test_public_key"`
