...
func fromQueueTransaction(tx queue.Transaction) *transactionsQueueRow {
	return &transactionsQueueRow{
		...
		Amount:           tx.Amount,
		TestPublicKey: tx.TestPublicKey,
	}
}
...
func (d *PostgresDatabase) CreateAddressAssociation(chain Chain, testAddress, address string, addressIndex uint32) error {
...
func (d *PostgresDatabase) GetAssociationByTestPublicKey(testPublicKey string) (*AddressAssociation, error) {
	addressAssociationTable := d.getTable(addressAssociationTableName, nil)
	row := &AddressAssociation{}
	where := map[string]interface{}{"test_public_key": testPublicKey}
	err := addressAssociationTable.Get(row, where).Exec()
	if err != nil {
		switch errors.Cause(err) {
		case sql.ErrNoRows:
