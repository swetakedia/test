...
func (ac *AccountConfigurator) Start() error {
	ac.log = common.CreateLogger("TestAccountConfigurator")
	ac.log.Info("TestAccountConfigurator starting")
	...
	root, err := ac.TestHorizon.Root()
	if err != nil {
		err = errors.Wrap(err, "Error loading TestHorizon root")
		ac.log.Error(err)
		return err
	}
...
func (ac *AccountConfigurator) ConfigureAccount(destination, assetCode, amount string) {
	...
	localLog.Info("Configuring Test account")
	...
	for {
		...
		if err != nil {
			localLog.WithField("err", err).Error("Error loading account from TestHorizon")
		...
		localLog.WithField("destination", destination).Info("Creating Test account")
		err = ac.createAccountTransaction(destination)
		if err != nil {
			localLog.WithField("err", err).Error("Error creating Test account")
			time.Sleep(2 * time.Second)
			continue
		}
	...
	for {
		account, err := ac.TestHorizon.LoadAccount(destination)
		if err != nil {
...
func (ac *AccountConfigurator) getAccount(account string) (testhorizon.Account, bool, error) {
	var hAccount testhorizon.Account
	hAccount, err := ac.TestHorizon.LoadAccount(account)
	if err != nil {
		if err2, ok := err.(*testhorizon.Error); ok && err2.Response.StatusCode == http.StatusNotFound {
...
func (ac *AccountConfigurator) signerExistsOnly(account testhorizon.Account) bool {
