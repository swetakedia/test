...
Bifrost is highly available and secure Bitcoin/Ethereum → Test bridge. It allows users to move BTC/ETH to Test network and then trade them to other tokens or participate in ICOs (initial coin offering).

It solves many problems connected to moving tokens to Test network:

* Security:
  * Developers don’t have access to users’ Test keys.
...
Download the binary from [the release page](https://github.com/test/go/releases/tag/bifrost-v0.0.2) and use it with it's [Bifrost JS SDK](https://github.com/test/bifrost-js-sdk).

## How it works

1. User opens the web app implemented using [Bifrost JS SDK](https://github.com/test/bifrost-js-sdk).
1. User is presented with her public and private Test keys where Bitcoin/Ethereum will be sent.
1. User selects what cryptocurrency she wants to move to Test network.
1. A receiving Bitcoin/Ethereum address is generated.
1. User sends funds in Bitcoin/Ethereum network.
1. Bifrost listens to Bitcoin and Ethereum network events. When payment arrives it creates a Test [account](https://www.test.org/developers/guides/concepts/accounts.html) for the user who later adds a temporary signer on the account.
1. Using a temporary signer Bifrost creates necessary [trust lines](https://www.test.org/developers/guides/concepts/assets.html) and sends corresponding amount of BTC/ETH to user's Test account. It also exchanges BTC/ETH to the final token at a given rate.
...
1. Now open Bifrost demo: https://bifrost.test.org/ It will display an Ethereum address for sending your test ETH.
...
* `bitcoin`
  ...
  * `token_price` - a price of one BTC in terms of final token (`test.token_asset_code`)
* `ethereum`
  ...
  * `token_price` - a price of one ETH in terms of final token (`test.token_asset_code`)
* `test`
  ...
  * `signer_secret_key` - distribuions accounts's secret key if only one instance of Bifrost is deployed OR [channel](https://www.test.org/developers/guides/channels.html)'s secret key if more than one instance of Bifrost is deployed, signer is also used as a temporary signer in new accounts (see "Account configuration" section)
  * `testhorizon` - URL to [testhorizon](https://github.com/test/go/tree/master/services/testhorizon) server
  * `network_passphrase` - Test network passphrase (`Public Global Test Network ; September 2015` for production network, `Test SDF Network ; September 2015` for test network)
  * `starting_balance` - Test XLM amount issued to created account (41 by default)
...
This is the most basic configuration but it is not recommended. In this configuration only one account is responsible for everything: issuing and distributing assets and creating new accounts. In this configuration `test.issuer_public_key`, `test.distribution_public_key` and `test.signer_secret_key` represent a single account.

### Issuing and Distribution Accounts

Sometimes you want to have a limited supply of your token so you create a distribution account, send all the tokens to distribution (and other accounts, ex. vesting) and lock the issuing account so no new tokens can be created. In this configuration `test.distribution_public_key` and `test.signer_secret_key` represent a separate account used to distribute funds in a token sale. This account will also be used to create new accounts.

### Distribution Account with multiple signers

If you expect really high load you should deploy multiple Bifrost servers. To prevent Bifrost instances from consuming the same sequence numbers of the Distribution account you should use [channels](https://www.test.org/developers/guides/channels.html). Each Bifrost server will contain a secret key of one of the signers of Distribution account and will use Signer account as a [transaction source](https://www.test.org/developers/guides/concepts/transactions.html#source-account) when creating new accounts.

In this configuration all servers have the same `test.issuer_public_key` and `test.distribution_public_key` values but each server has a separate `test.signer_secret_key` which is a signer of `test.distribution_public_key` account (and needs to exist in Test ledger).
...
* Make sure "Sell [your token] for BTC" and/or "Sell [your token] for ETH" exist in Test production network. If not, you can create an offer by sending a transaction with `manage_offer` operation.
* Make sure you don't use account from `test.issuer_secret_key` anywhere else than bifrost. Otherwise, sequence numbers will go out of sync and bifrost will stop working. It's good idea to create a new signer on issuing account.
...
* Deploy at least 2 bifrost, bitcoin-core, geth, test-core and testhorizon servers. Use multi-AZ database.
* Do not use SDF's testhorizon servers. There is no SLA and we cannot guarantee it will handle your load.
* Make sure bifrost <-> bitcoin-core and bifrost <-> geth connections are not public or are encrypted (mitm attacks).
* It's a good idea to set "Authorization required" and "Authorization revocable" [flags](https://www.test.org/developers/guides/concepts/accounts.html#flags) on issuing account during ICO stage to remove trustlines to accounts with lost keys. You need to set `test.needs_authorize` in config to `true` if you want to use this feature.
* Monitor bifrost logs and react to all WARN and ERROR entries.
* Make sure you are using geth >= 1.7.1 and bitcoin-core >= 0.15.0.
* Increase testhorizon rate limiting to handle expected load.
...
* `Bifrost.ExchangedTimelockedEvent` [event](https://github.com/test/bifrost-js-sdk#oneventevent-data). You can display it to the user if you want your users to unlock accounts.
...
Check [Test Stack Exchange](https://test.stackexchange.com/questions/tagged/bifrost/) to find similar questions or add the new question with the `bifrost` tag.
