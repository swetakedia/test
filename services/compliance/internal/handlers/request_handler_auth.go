...
import (
	...
	baseAmount "github.com/test/go/amount"
	"github.com/test/go/protocols/compliance"
	"github.com/test/go/services/compliance/internal/db"
	shared "github.com/test/go/services/internal/bridge-compliance-shared"
	httpHelpers "github.com/test/go/services/internal/bridge-compliance-shared/http/helpers"
	callback "github.com/test/go/services/internal/bridge-compliance-shared/protocols/compliance"
...
func (rh *RequestHandler) HandlerAuth(w http.ResponseWriter, r *http.Request) {
	...
	senderTestToml, err := rh.TestTomlResolver.GetTestTomlByAddress(authData.Sender)
	if err != nil {
		log.WithFields(log.Fields{"err": err, "sender": authData.Sender}).Warn("Cannot get test.toml of sender")
		errorResponse := httpHelpers.NewInvalidParameterError("data.sender", "Cannot get test.toml of sender")
		httpHelpers.Write(w, errorResponse)
		return
	}

	if !shared.IsValidAccountID(senderTestToml.SigningKey) {
		errorResponse := httpHelpers.NewInvalidParameterError("data.sender", "SIGNING_KEY in test.toml of sender is invalid")
		// TODO
		// log.WithFields(errorResponse.LogData).Warn("SIGNING_KEY in test.toml of sender is invalid")
		httpHelpers.Write(w, errorResponse)
		return
	}
	...
	err = rh.SignatureSignerVerifier.Verify(senderTestToml.SigningKey, []byte(authreq.DataJSON), signatureBytes)
	if err != nil {
		log.WithFields(log.Fields{
			"signing_key": senderTestToml.SigningKey,
			"data":        authreq.Data,
