...
func (rh *RequestHandler) HandlerSend(w http.ResponseWriter, r *http.Request) {
	...
	if authDataEntity != nil {
		var testToml *testtoml.Response
		testToml, err = rh.TestTomlResolver.GetTestToml(authDataEntity.Domain)
		if err != nil {
		...
		if testToml.AuthServer == "" {
			log.Print("No AUTH_SERVER in test.toml")
			helpers.Write(w, callback.AuthServerNotDefined)
			return
		}

		rh.sendAuthData(w, testToml.AuthServer, []byte(authDataEntity.AuthData))
		return
	}
	...
	testToml, err := rh.TestTomlResolver.GetTestToml(domain)
	if err != nil {
		log.WithFields(log.Fields{
			"destination": request.Destination,
			"err":         err,
	...
	rh.sendAuthData(w, testToml.AuthServer, data)
}
