...
This is a small tool, written in Go, for working with `test-core` history archives directly.
It is a standalone tool that does not require `test-core`, or any other programs.

It is much smaller and simpler than `test-core`, and is intended only for archive-maintenance tasks.
...
```
$ go install github.com/test/go/tools/test-archivist
```

## Usage

```
inspect test history archive

Usage:
  test-archivist [flags]
  test-archivist [command]
...
Flags:
  ...
  -h, --help              help for test-archivist
...
Use "test-archivist [command] --help" for more information about a command.
```

## Specifying history archives

Unlike `test-core`, `test-archivist` does not run subprocesses to access history archives;
...
`test-archivist` supports reading from and writing to any S3-compatible storage.
...
```
$ test-archivist status --s3endpoint ams3.digitaloceanspaces.com s3://bucketname/prefix
```
...
```
$ test-archivist status http://s3-eu-west-1.amazonaws.com/history.test.org/prd/core-testnet/core_testnet_001

       Archive: http://s3-eu-west-1.amazonaws.com/history.test.org/prd/core-testnet/core_testnet_001
...
```
$ test-archivist mirror http://s3-eu-west-1.amazonaws.com/history.test.org/prd/core-testnet/core_testnet_001 file://local-archive

2016/02/10 18:27:09 mirroring http://s3-eu-west-1.amazonaws.com/history.test.org/prd/core-testnet/core_testnet_001 -> file://local-archive
...
```
$ test-archivist --last 1024 mirror http://s3-eu-west-1.amazonaws.com/history.test.org/prd/core-testnet/core_testnet_001 file://local-archive

2016/02/10 19:14:01 mirroring http://s3-eu-west-1.amazonaws.com/history.test.org/prd/core-testnet/core_testnet_001 -> file://local-archive
...
```
$ test-archivist scan file://local-archive
...
```
$ test-archivist --last 4096 scan file://local-archive
...
$ test-archivist --low 1000000 --high 1006000 scan file://local-archive
...
```
$ test-archivist --verify --last 4096 scan file://local-archive
...
$ test-archivist repair file://local-archive file://broken-archive
...
```
 test-archivist dumpxdr local-archive/transactions//00/20/de/transactions-0020de7f.xdr.gz
