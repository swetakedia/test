...
[![GoDoc](https://godoc.org/github.com/test/go?status.svg)](https://godoc.org/github.com/test/go)
[![Go Report Card](https://goreportcard.com/badge/github.com/test/go)](https://goreportcard.com/report/github.com/test/go)

This repo is the home for all of the public go code produced by SDF.  In addition to various tools and services, this repository is the SDK from which you may develop your own applications that integrate with the test network.

## Package Index

* [TestHorizon Server](services/testhorizon): Full-featured API server for Test network
* [Go Clients (TestHorizon SDK)](clients): Go SDK for making requests to TestHorizon Server
* [Bifrost](services/bifrost): Bitcoin/Ethereum -> Test bridge
...
You can use dep yourself in your project and add test go as a vendor'd dependency, or you can just drop this repos as `$GOPATH/src/github.com/test/go` to import it the canonical way (you still need to run `dep ensure -v`).
...
* **clients** contains packages that provide client packages to the various Test services.
* **exp** contains experimental packages.  Use at your own risk.
* **handlers** contains packages that provide pluggable implementors of `http.Handler` that make it easier to incorporate portions of the Test protocol into your own http server. 
* **support** contains packages that are not intended for consumption outside of Test's other packages.  Packages that provide common infrastructure for use in our services and tools should go here, such as `db` or `log`. 
...
In addition to the packages described above, this repository contains various packages related to working with the Test network from a go program.  It's recommended that you use [godoc](https://godoc.org/github.com/test/go#pkg-subdirectories) to browse the documentation for each.
