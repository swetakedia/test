...
// the format used internally to test-core.
//
// test-core represents asset "amounts" as 64-bit integers, but to enable
// fractional units of an asset, testhorizon, the client-libraries and other built
// on top of test-core use a convention, encoding amounts as a string of
// decimal digits with up to seven digits of precision in the fractional
// portion. For example, an amount shown as "101.001" in testhorizon would be
// represented in test-core as 1010010000.
package amount
...
// One is the value of one whole unit of currency. Test uses 7 fixed digits
...
var (
	...
	// Note: {1,20} because the biggest amount you can use in Test is:
...
// Parse parses the provided as a test "amount", i.e. a 64-bit signed integer
...
// ParseInt64 parses the provided as a test "amount", i.e. a 64-bit signed
...
// IntStringToAmount converts string integer value and converts it to test
