// Package testhorizon provides client access to a testhorizon server, allowing an
...
var DefaultTestNetClient = &Client{
	URL:  "https://testhorizon-testnet.test.org",
...
// remote method call to testhorizon.
type At string
...
// Client struct contains data required to connect to TestHorizon instance
// It is okay to call methods on Client concurrently.
// A Client must not be copied after first use.
type Client struct {
	// URL of TestHorizon server to connect
	URL string
...
// Error struct contains the problem returned by TestHorizon
type Error struct {
	Response *http.Response
	Problem  Problem
}

// HTTP represents the HTTP client that a testhorizon client uses to communicate
type HTTP interface {
...
// ensure that the testhorizon client can be used as a SequenceProvider
var _ build.SequenceProvider = &Client{}

// ensure that the testhorizon client implements ClientInterface
