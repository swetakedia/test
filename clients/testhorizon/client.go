...
// Root loads the root endpoint of testhorizon
...
// LoadAccount loads the account state from testhorizon. err can be either error
// object or testhorizon.Error object.
...
// LoadAccountOffers loads the account offers from testhorizon. err can be either
// error object or testhorizon.Error object.
...
// LoadTradeAggregations loads the trade aggregation from testhorizon.
...
// LoadTrades loads the /trades endpoint from testhorizon.
...
// LoadTransaction loads a single transaction from TestHorizon server
...
// LoadOperation loads a single operation from TestHorizon server
			...
			for {
				...
				if err != nil {
					if err == io.EOF || err == io.ErrUnexpectedEOF {
						...
						//   happen in TestHorizon as it always sends an empty line after each event.
						// - The stream was closed by the server/proxy because the connection was idle.
						//
						// In the former case, that (again) should never happen in TestHorizon, we need to
						// check if there are any events we need to decode. We do this in the `if`
						// statement below just in case if TestHorizon behaviour changes in a future.
						//
						// From spec:
...
// SubmitTransaction submits a transaction to the network. err can be either error object or testhorizon.Error object.
