...
// LookupByAddress performs a federated lookup following to the test
...
// "name" type is a legacy holdover from the legacy test network's federation
...
// LookupByAccountID performs a federated lookup following to the test
...
func (c *Client) LookupByAccountID(aid string) (*proto.IDResponse, error) {
	...
	domain, err := c.TestHorizon.HomeDomainForAccount(aid)
	if err != nil {
...
// ForwardRequest performs a federated lookup following to the test
...
func (c *Client) getFederationServer(domain string) (string, error) {
	stoml, err := c.TestTOML.GetTestToml(domain)
	if err != nil {
		return "", errors.Wrap(err, "get test.toml failed")
	}

	if stoml.FederationServer == "" {
		return "", errors.New("test.toml is missing federation server info")
	}
