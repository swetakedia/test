...
// test-core server using HTTP
type Client struct {
	// HTTP is the client to use when communicating with test-core.  If nil,
	// http.DefaultClient will be used.
	HTTP HTTP

	// URL of Test Core server to connect.
	URL string
}

// Info calls the `info` command on the connected test core and returns the
// provided response
...
// SetCursor calls the `setcursor` command on the connected test core
func (c *Client) SetCursor(ctx context.Context, id string, cursor int32) error {
	...
	if body != SetCursorDone {
		return errors.Errorf("failed to set cursor on test-core: %s", body)
	}

	return nil
}

// SubmitTransaction calls the `tx` command on the connected test core with the provided envelope
...
// WaitForNetworkSync continually polls the connected test-core until it
...
// simpleGet returns a new GET request to the connected test-core using the
