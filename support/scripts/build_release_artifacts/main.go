...
func build(pkg, dest, version, buildOS, buildArch string) {
	...
	timeFlag := fmt.Sprintf("-X github.com/test/go/support/app.buildTime=%s", buildTime)
	versionFlag := fmt.Sprintf("-X github.com/test/go/support/app.version=%s", version)
...
// Tags must be of the form `NAME-vSEMVER`, such as `testhorizon-v1.0.0` to be
