...
// to sender server in order to obtain test.toml file and signing key.
func (r *AuthRequest) VerifySignature(sender string) error {
	...
	senderTestToml, err := testtoml.GetTestTomlByAddress(sender)
	if err != nil {
		return errors.Wrap(err, "Cannot get test.toml of sender domain")
	}

	if senderTestToml.SigningKey == "" {
		return errors.New("No SIGNING_KEY in test.toml of sender")
	}

	kp, err := keypair.Parse(senderTestToml.SigningKey)
	if err != nil {
