...
func TestValidateError(t *testing.T) {
	...
	authData := &AuthData{
		Sender:         "bartektest.org",
		NeedInfo:       false,
		Tx:             "&^%",
		AttachmentJSON: "abc",
	}

	assert.EqualError(t, authData.Validate(), "sender: bartektest.org does not validate as test_address;tx: &^% does not validate as base64;attachment: abc does not validate as json")
}
