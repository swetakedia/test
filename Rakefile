...
namespace :xdr do
  ...
  # As test-core adds more .x files, we'll need to update this array
  # Prior to launch, we should be separating our .x files into a separate
  # repo, and should be able to improve this integration.
  HAYASHI_XDR = [
                 "src/xdr/Test-types.x",
                 "src/xdr/Test-ledger-entries.x",
                 "src/xdr/Test-transaction.x",
                 "src/xdr/Test-ledger.x",
                 "src/xdr/Test-overlay.x",
                 "src/xdr/Test-SCP.x",
                ]
  ...
  task :download do
    ...
    HAYASHI_XDR.each do |src|
      ...
      encoded    = client.contents("test/test-core", path: src).content
